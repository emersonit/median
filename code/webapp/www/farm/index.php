...
if ($action == 'h') {
	...
	if (!isset($farmer)) {
		...
		$new_farmer['tsc'] = time();
		$new_farmer['tsh'] = time();
		$insert = $farmdb->farmers->insert($new_farmer, array('w' => 1));
	} else {
		// ok, update its heartbeat
		$update = $farmdb->farmers->update(array('ip' => $farmer_ip), array('$set' => array('tsh' => time())), array('w' => 1));
	}
	...
	if ($get_jobs->count() > 0) {
		...
		$update_job = $farmdb->jobs->update(array('_id' => $job['_id']), array('$set' => $updated_job), array('w' => 1));
	} else {
		$job['jobs'] = 0;
	}
	...
	$update_job = $farmdb->jobs->update(array('_id' => $jobid), array('$set' => $updated_job), array('w' => 1));
	...
	if ($json['s'] * 1 == 2) {
		...
		foreach ($media_entry['pa']['c'] as $media_path) {
			...
			if (trim($new_media_path['p']) == trim($job['out'])) {
				...
				$new_media_path['fs'] = (int) $json['m'] * 1; // filesize will be returned with the JSON from the farmer
			}
			$updated_media_entry['pa']['c'][] = $new_media_path;
		}
		// update!
		$update_media = $mdb->media->update(array('mid' => $mid), array('$set' => $updated_media_entry), array('w' => 1));
